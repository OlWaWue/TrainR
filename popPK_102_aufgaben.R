...
## Lade die Bibliothek für schöne Abbildungen
...
ETA_ke <- rnorm(__________)

ke <- ke_pop * exp(__________)

dat <- __________(dat, ke)
...
ETA_ka <- rnorm(__________)

ka <- __________ * exp(ETA_ka)

dat <- __________(dat, ka)



## Jetzt schauen wir uns die weiteren Histogramme mit ihrer Dichteverteilung an

pl3 <- ggplot(data = dat) + __________(aes(x=ETA_ke, y=..density..), fill="white", colour="black") + 
  geom_density(aes(y=..density.., x=ETA_ke)) + theme_bw()



pl4 <- ggplot(data = dat) + __________(aes(x=ke, y=..density..), fill="white", colour="black") + 
  geom_density(aes(y=..density.., x=ke)) + theme_bw()


pl5 <- ggplot(data = dat) + __________(aes(x=ETA_ka, y=..density..), fill="white", colour="black") + 
  geom_density(aes(y=..density.., x=ETA_ka)) + theme_bw()


pl6 <- ggplot(data = dat) + __________(aes(x=ka, y=..density..), fill="white", colour="black") + 
  geom_density(aes(y=..density.., x=ka)) + theme_bw()

## Der Befehl stellt die Plots in einem Muster 2 mal 3 dar
...
#
##
## Lasst uns schauen, ob die zufälligen Effekte korrelieren => Scatterplot erzeugen

pl7 <- ggplot(data = dat) + __________(aes(x=ETA_ka, y=ETA_ke)) + theme_bw()
pl8 <- ggplot(data = dat) + __________(aes(x=ETA_ka, y=ETA_VC)) + theme_bw()
pl9 <- ggplot(data = dat) + __________(aes(x=ETA_ke, y=ETA_VC)) + theme_bw()
...
## Eine Zeitsequenz erzeugen => Zu diesen Zeit-Werten sollen PK-Spiegel simuliert werden
times = __________(from=0, to=24, by=0.1)
...
dat_mc <- NULL
  ...
  ##  Tausend Simulationen mit der Dosis 200mg und und 87% systemisch verfügbarem Anteil
  for(i in 1:__________){

      CP_mc <-  pk_model_ss(AMT=__________, 
                            F_oral=__________, 
                            dat$Vc[i], 
                            dat$ke[i], 
                            dat$ka[i], 
...
s <- apply(__________,2,function(x) quantile(x,probs=c(0.05,0.1,0.15,0.2,0.25,0.5,0.75,0.8,0.85,0.9,0.95)))
...
plot_dat <- data.frame(TIME=times, CP_min_1=s[1,], CP_min_2=s[2,], CP_min_3=s[3,], CP_min_4=s[4,], CP_min_5=s[5,], ## untere Perzentile
                       CP=__________, ## Median
                       CP_max_1=s[7,], CP_max_2=s[8,],CP_max_3=s[9,],CP_max_4=s[10,],CP_max_5=s[11,]) ## obere Perzentile

my_beautiful_colour = "red"


## Bitte den Plot mit "Bändern" welche die jeweiligen Vorhersageintervalle vom 5-95% Perzentil, 10-90% Perzentil,
## 15-85% Perzentil, 20-80% Perzentil, 25-75% Perzentil einspannen,
## Farbe für den Median und die Bänder soll "my_beautiful_colour" sein

pl <- ggplot(plot_dat) + geom_line(aes(x=TIME, y=CP), colour=__________, size=0.75) + 
  __________(aes(x=TIME, ymax=CP_max_5, ymin=CP_min_1), alpha=0.15, fill=__________) +
  __________(aes(x=TIME, ymax=CP_max_4, ymin=CP_min_2), alpha=0.15, fill=__________) +
  __________(aes(x=TIME, ymax=CP_max_3, ymin=CP_min_3), alpha=0.15, fill=__________) +
  __________(aes(x=TIME, ymax=CP_max_2, ymin=CP_min_4), alpha=0.15, fill=__________) +
  __________(aes(x=TIME, ymax=CP_max_1, ymin=CP_min_5), alpha=0.15, fill=__________) +
